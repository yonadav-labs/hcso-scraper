...
run-cron:
	docker run --rm -d --name scraper-cron --restart=unless-stopped gregorywhite/scraper-cron

run-web:
	docker run --rm -d --name scraper-web --restart=unless-stopped -p 8000:8000 gregorywhite/scraper-web
